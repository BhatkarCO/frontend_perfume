...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      toast.success(result.message || 'Registration successful. Verify email to complete.');
    ...
    <div className="min-h-[85vh] flex items-center justify-center px-4 py-16 bg-luxury-deep">
      <div className="w-full max-w-md p-8 bg-white rounded-sm border border-luxury-lightgrey shadow-md flex flex-col gap-6">
        ...
        <div className="text-center flex flex-col items-center gap-1">
          <span className="font-playfair text-xl font-bold tracking-widest text-luxury-black">BHATKAR & CO. PERFUMES</span>
          <h2 className="text-[10px] font-bold tracking-wider text-gray-400 uppercase mt-2">Create An Account</h2>
          <p className="text-xs text-gray-500 font-light">Join the Bhatkar & Co. inner circle for exclusive updates.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <p className="text-[10px] text-gray-500 leading-relaxed font-light">
            By signing up, you agree to Bhatkar & Co.'s <Link href="/policy/terms" className="text-gold underline">Terms & Conditions</Link> and <Link href="/policy/privacy" className="text-gold underline">Privacy Policy</Link>.
          </p>
        ...
        <p className="text-center text-xs text-gray-500 font-light">
          ...
          <Link href="/login" className="text-gold hover:text-gold-dark font-bold uppercase tracking-wider text-[10px]">
            Sign In
          </Link>
        </p>
